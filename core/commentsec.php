<?php Typecho_Plugin::factory('Widget_Feedback')->comment = array('BearcoolSpam', 'filter');
...
class BearcoolSpam
{
    ...
    public static function filter($comment, $post)
    {
        ...
        if (!empty($options->BearSpam_NAMEMIN)) {
            if (BearcoolSpam::strLength($comment['author']) < $options->BearSpam_NAMEMIN) {
        ...
        if (!empty($options->BearSpam_MIN)) {
            if (BearcoolSpam::strLength($comment['text']) < $options->BearSpam_MIN) {
                throw new Typecho_Widget_Exception("抱歉，系统检测到您的评论内容字数过少，已自动拦截。");
                Typecho_Cookie::set('__typecho_remember_text', $comment['text']);
            }
        }
        if (!empty($options->BearSpam_MAX)) {
            if (BearcoolSpam::strLength($comment['text']) > $options->BearSpam_MAX) {
                throw new Typecho_Widget_Exception("抱歉，系统检测到您的评论内容字数过多，已自动拦截。");
                Typecho_Cookie::set('__typecho_remember_text', $comment['text']);
            }
        }
        if (!empty($options->BearSpam_Words)) {
            if (BearcoolSpam::check_in($options->BearSpam_Words, $comment['text'])) {
